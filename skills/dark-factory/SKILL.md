...
| **Cross-step artifact survival** | **For every computed value/report/artifact one step produces, what happens to it if a LATER, unrelated step fails? Does it get written/logged somewhere even when the pipeline can't fully complete, or does it silently vanish with no trace?** Added 2026-09-14 (`consolidate` mode, nuwa): the exact same gap — a computed report discarded with no warning when video capture failed — was found and fixed twice, independently, in the same project (`demo_report`, then `decision_trace_report`), because nothing in a spec author's checklist prompted asking this question up front; both times it was caught by review, not designed against. |
