...
- **When a schema is defined independently of its TS type (not derived via `z.infer` — e.g. a hand-written interface with a parallel zod schema), every field added to one MUST be added to the other in the same change, and the change needs a test that fails if they drift.** Added 2026-09-08 after a real instance: `component-graph.tsx`'s `GraphNode` interface gained two fields never mirrored into `foreman.ts`'s `PhaseGraphNodeSchema` — zod's default is to silently strip unknown keys, so the feature compiled and rendered while quietly dropping the exact data it existed to show, caught only by live click-testing. Prefer `z.infer` in the first place so this class of drift is structurally impossible; when that's not the shape (e.g. the type is the source of truth for a UI-only prop set), name explicitly which side is derived from which.
